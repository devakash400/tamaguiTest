...
const { Provider: AccordionValueProvider, useStyledContext: useAccordionValueContext } = (0, import_web2.createStyledContext)(), {
  Provider: AccordionCollapsibleProvider,
  useStyledContext: useAccordionCollapsibleContext
} = (0, import_web2.createStyledContext)(), AccordionImplSingle = React.forwardRef((props, forwardedRef) => {
  const {
    value: valueProp,
    defaultValue,
    control,
...
}), { Provider: AccordionImplProvider, useStyledContext: useAccordionContext } = (0, import_web2.createStyledContext)(), AccordionImpl = React.forwardRef(
  (props, forwardedRef) => {
    const {
      __scopeAccordion,
      disabled,
      ...
      ...accordionProps
    } = props, accordionRef = React.useRef(null), composedRef = (0, import_compose_refs.useComposedRefs)(accordionRef, forwardedRef), getItems = useCollection(__scopeAccordion || ACCORDION_CONTEXT), isDirectionLTR = (0, import_use_direction.useDirection)(dir) === "ltr", handleKeyDown = (0, import_helpers.composeEventHandlers)(
      props.onKeyDown,
      (event) => {
...
), ITEM_NAME = "AccordionItem", { Provider: AccordionItemProvider, useStyledContext: useAccordionItemContext } = (0, import_web2.createStyledContext)(), AccordionItem = React.forwardRef(
  (props, forwardedRef) => {
...
const AccordionTriggerFrame = (0, import_web2.styled)(import_collapsible.Collapsible.Trigger, {
  variants: {
    unstyled: {
      false: {
        cursor: "pointer",
...
}), HeightAnimator = import_web.View.styleable((props, ref) => {
  const itemContext = useAccordionItemContext(), { children, ...rest } = props, [height, setHeight] = React.useState(0);
  React.useEffect(() => {
    itemContext.open || setHeight(0);
  }, [itemContext.open]);
  const onLayout = (0, import_web.useEvent)(({ nativeEvent }) => {
    nativeEvent.layout.height && setHeight(nativeEvent.layout.height);
  });
  return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(import_web.View, { ref, height, ...rest, children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
    import_web.View,
    {
      position: "absolute",
      onLayout,
