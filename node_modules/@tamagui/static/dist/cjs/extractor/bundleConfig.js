...
async function bundleConfig(props) {
  try {
    ...
    const configEntry = props.config ? (0, import_getTamaguiConfigPathFromOptionsConfig.getTamaguiConfigPathFromOptionsConfig)(props.config) : "", tmpDir = (0, import_node_path.join)(process.cwd(), ".tamagui"), configOutPath = (0, import_node_path.join)(tmpDir, "tamagui.config.cjs"), baseComponents = (props.components || []).filter((x) => x !== "@tamagui/core"), componentOutPaths = baseComponents.map(
      (componentModule) => (0, import_node_path.join)(
        tmpDir,
        `${componentModule.split(import_node_path.sep).join("-").replace(/[^a-z0-9]+/gi, "")}-components.config.cjs`
      )
    );
    ...
    \u27A1 [tamagui] built config and components (${Date.now() - start}ms):`
        ...
        Components ${[
          ...componentOutPaths.map((p) => `.${import_node_path.sep}${(0, import_node_path.relative)(process.cwd(), p)}`)
        ].join(`
             `)}
        `
      );
...
function loadComponentsInner(props, forceExports = !1) {
  ...
  try {
    const info = componentsModules.flatMap((name) => {
      const isDynamic = !!(0, import_node_path.extname)(name) && forceExports, fileContents = isDynamic ? (0, import_node_fs.readFileSync)(name, "utf-8") : "", loadModule = isDynamic ? (0, import_node_path.join)((0, import_node_path.dirname)(name), `.tamagui-dynamic-eval-${(0, import_node_path.basename)(name)}.tsx`) : name;
